...
void ComponentCamera::Update(float) {
	...
	auto _transform = gameObject->GetTransform();
	glm::vec3 position, scale;
	glm::quat _rotation;

	// we don't really care about these for our gameObjects, but we need to matck glm::decompose() signature
	glm::vec3 skew; glm::vec4 perspective;

	glm::decompose(_transform, scale, _rotation, position, skew, perspective);

	_camera->setPositionAndRotation(position, glm::degrees(glm::eulerAngles(_rotation)));
}
